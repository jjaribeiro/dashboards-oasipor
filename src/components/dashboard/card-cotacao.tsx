...
import { ESTADO_COTACAO } from "@/lib/constants";
import type { Cotacao } from "@/lib/types";

interface CardCotacaoProps {
  item: Cotacao;
  onComplete: (id: string) => void;
  onEdit: (item: Cotacao) => void;
  completing?: boolean;
}

export function CardCotacao({ item, onComplete, onEdit, completing }: CardCotacaoProps) {
  const urgency = getUrgencyLevel(item.prazo);
  const urgencyColor = getUrgencyColor(urgency);

  const estadoColor =
    item.estado === "follow_up"
    ...
    >
      {item.numero_dossier && (
        <div className="mb-1.5 flex items-center gap-1.5">
          <span className="inline-flex items-center rounded-md border border-purple-300 bg-purple-100 px-2 py-0.5 text-xs font-extrabold uppercase tracking-wide text-purple-800">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        ...
        <span className={cn("inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-bold", estadoColor)}>
          {ESTADO_COTACAO[item.estado]}
        </span>
      </div>
