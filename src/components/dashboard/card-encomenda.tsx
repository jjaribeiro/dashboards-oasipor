...
interface CardEncomendaProps {
  item: Encomenda;
  onComplete: (id: string) => void;
  onEdit: (item: Encomenda) => void;
  completing?: boolean;
}

export function CardEncomenda({ item, onComplete, onEdit, completing }: CardEncomendaProps) {
  const urgency = getUrgencyLevel(item.data_encomenda);
  const urgencyColor = item.data_encomenda
  ...
  return (
    <div
      className={cn(
        ...
        urgencyColor,
        urgency === "overdue" && item.data_encomenda && "pulse-overdue",
        completing && "animate-fade-out"
      )}
      onClick={() => onEdit(item)}
    >
      {/* Dossier + Encomenda Cliente destacados no topo */}
      {(item.numero_dossier || item.numero_encomenda) && (
        <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
          {item.numero_dossier && (
            <span className="inline-flex items-center rounded-md border border-amber-300 bg-amber-100 px-2 py-0.5 text-xs font-extrabold uppercase tracking-wide text-amber-800">
          ...
          {item.numero_encomenda && (
            <span className="inline-flex items-center rounded-md border border-slate-300 bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-700">
              Enc. Cliente Nº {item.numero_encomenda}
            </span>
          )}
        </div>
      )}
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        ...
        >
          {daysLabel(item.data_encomenda)}
        </span>
        {item.estado && (
          <span className="inline-flex items-center rounded-md border border-amber-200 bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-700">
            {item.estado}
          </span>
        )}
      </div>
      ...
      {item.data_encomenda && (
        <p className="mt-1.5 text-xs font-bold text-slate-500">
          Data-Limite: {formatDate(item.data_encomenda)}
        </p>
      )}
